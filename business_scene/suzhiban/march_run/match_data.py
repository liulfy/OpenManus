...
file_name = "business_scene/suzhiban/march_run/szb_3月_new.xlsx"
...
def run_total_inference(region, extract_content, prod_one_desc, prod_num_new, rule_set):
    ...
    def _run_2(result, region, extract_content, index):
        result[index] = get_result(run_pipeline(extract_content, prod_num_new, region, prod_one_desc))
...
def run_row_data(df, rule_set, result, start_index, end_index):
    for i in range(start_index, end_index):
        ...
        inference_result = run_total_inference(region, extract_content, prod_one_desc, prod_num_new, rule_set)
